...
const verifyLoginValidation = [
  query("email").isEmail().normalizeEmail(), //Kiểm tra định dạng
  query("id").isLength({ min: 24, max: 24 }), //id bắt buộc phải dài đúng 24
  (req, res, next) => {
...
const sendLoginVerificationEmail = async (req, res) => {
  ...
  try {
    ...
    res.status(401).json({
      message: `Access blocked due to suspicious activity. Verification emal was sent to your email address.`,
    });
  } catch (err) {
    console.log(
